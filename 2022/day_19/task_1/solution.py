...
CacheKey: TypeAlias = tuple[int, int, int, int, int, int, int, int]
...
def result(input_file: str) -> int:
    ...
    for i, line in enumerate(input_file.splitlines()):
        ...
        quality_level_sum += (i + 1) * max_geodes_opened

    return quality_level_sum


def solve(input_file: str) -> str:
    return f"The sum of quality levels is: {result(input_file)}"
