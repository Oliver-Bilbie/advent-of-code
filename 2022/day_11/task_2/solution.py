...
class Monkey:
    ...
    def inspect_item(self, lcm: int) -> InspectionResult:
        worry_level = self._operation(self._items.popleft()) % lcm
...
def result(input_file: str) -> int:
    ...
    divisor_lcm = math.lcm(*set(m.divisor for m in monkeys))

    for _round in range(10000):
        for m in monkeys:
            while m.has_items():
                r = m.inspect_item(divisor_lcm)
...
def solve(input_file: str) -> str:
    return f"The level of monkey business after 10000 rounds is: {result(input_file)}"
