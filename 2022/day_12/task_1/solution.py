...
def read_map(input_file: str) -> tuple[Tile, Tile]:
    ...
    for i in range(map_height):
        for j in range(map_width):
            t = grid[i][j]
            if i > 0 and grid[i - 1][j].height <= t.height + 1:
                t.neighbors.append(grid[i - 1][j])
            if i + 1 < map_height and grid[i + 1][j].height <= t.height + 1:
                t.neighbors.append(grid[i + 1][j])
            if j > 0 and grid[i][j - 1].height <= t.height + 1:
                t.neighbors.append(grid[i][j - 1])
            if j + 1 < map_width and grid[i][j + 1].height <= t.height + 1:
                t.neighbors.append(grid[i][j + 1])

    if start is None:
        raise ValueError("The map does not contain a starting location")
    if end is None:
...
def bfs(start: Tile, end: Tile) -> int:
    queue = deque([start])
    start.distance = 0
