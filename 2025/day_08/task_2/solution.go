...
func findConnections(points []Point) *PairHeap {
	h := &PairHeap{}
	heap.Init(h)
...
func findFinalBoxes(connections *PairHeap, pointsCount int) Pair {
	...
	for connections.Len() > 0 {
		...
		if len(groups) == 1 && len(groups[0]) == pointsCount {
			return pair
		}
	}

	panic("The groups were never connected")
}

func result(input string) uint64 {
	points := readPoints(input)
	connections := findConnections(points)
	pair := findFinalBoxes(connections, len(points))
	result := uint64(pair.a.x) * uint64(pair.b.x)
	return result
}
