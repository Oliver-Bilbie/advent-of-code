...
def result(input_file: str) -> int:
    ...
    for line in input_file.splitlines():
        ...
        # Diagonal
            ...
            x_step = 1 if rng.start[0] <= rng.end[0] else -1
            y_step = 1 if rng.start[1] <= rng.end[1] else -1
            step_total = abs(rng.end[0] - rng.start[0]) + 1
            for offset in range(step_total):
                x = rng.start[0] + x_step * offset
                y = rng.start[1] + y_step * offset
                vents[(x, y)] = vents.get((x, y), 0) + 1
