...
fn simulate_step(grid: &mut Grid<Option<u8>>) -> usize {
...
fn result(input: &str) -> u64 {
    ...
    let octopus_count = grid.size().row * grid.size().column;
    let mut step_count = 1;
    while simulate_step(&mut grid) != octopus_count {
        step_count += 1;
    }
    step_count
}
...
pub fn solve(input: &str) -> String {
    return format!("The octopuses synchronize on step {}", result(input));
}
