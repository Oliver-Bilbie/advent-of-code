...
fn read_lab(source_file: &str) -> Lab {
    ...
    let file_str = fs::read_to_string(source_file).unwrap();

    let tiles: Vec<Vec<Tile>> = file_str
        .lines()
        .map(|line| {
            line.chars()
                .map(|char| match char {
...
fn main() {
    let lab = read_lab("../input.txt");
    let possible_positions = find_positions(&lab);
    println!("There are {} possible positions", possible_positions);
}
...
mod tests {
    ...
    fn it_solves_the_example() {
        let lab = read_lab("../test_input.txt");
        let actual_positions = find_positions(&lab);
        let expected_positions = 6;
