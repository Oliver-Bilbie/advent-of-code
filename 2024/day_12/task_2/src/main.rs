...
fn read_garden(input_file: &str) -> Garden {
    let tiles: Vec<Vec<Plot>> = fs::read_to_string(input_file)
        .unwrap()
        .lines()
        .map(|line| {
            line.chars()
...
fn main() {
    let garden = read_garden("../input.txt");
    let fence_cost = calculate_fence_cost(&garden);
    println!("The cost of fences is is: {}", fence_cost);
}
...
mod tests {
    ...
    fn it_solves_the_example() {
        let garden = read_garden("../test_input.txt");
        println!("{:?}", garden.tiles);
        let actual_cost = calculate_fence_cost(&garden);
        let expected_cost = 1206;
